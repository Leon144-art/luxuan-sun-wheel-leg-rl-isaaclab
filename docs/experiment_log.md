...
| compat-001 | 2026-06-16 | pre-upload workspace | WSL2 GPU workstation | MuJoCo 3.9, MJX 3.9, MuJoCo Warp 3.9 | N/A | `python scripts/compat/mujoco_compat_test.py` and optional `--mjx` / `--warp` checks | <1 min after setup and cache warmup | Passed: MuJoCo stepping, offscreen render, MJX CPU execution, MuJoCo Warp GPU execution | `outputs/compat/mujoco/` | Viewer shutdown under WSLg still needs separate validation |
